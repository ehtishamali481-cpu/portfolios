...
import { Sun, Moon, Lock, Terminal, Menu, X } from 'lucide-react';
    ...
    <nav className="sticky top-0 z-50 w-full border-b border-zinc-200/80 bg-white/80 backdrop-blur-md transition-colors duration-300 dark:border-zinc-800/80 dark:bg-zinc-950/80">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
        ...
        >
          ...
          <span className="font-semibold">MERN Developer</span>
        </button>
        ...
        <div className="hidden items-center gap-1 md:flex">
          {navItems.map((item) => (
            <button
              ...
              className={`rounded-full px-4 py-1.5 text-sm font-medium transition-all cursor-pointer ${!showDashboard && activeSection === item.id
          ...
          {token ? (
            <div className="flex items-center gap-2">
              <button
                ...
                className={`flex items-center gap-1.5 rounded-lg border px-4 py-1.5 text-sm font-semibold transition-all cursor-pointer ${showDashboard
